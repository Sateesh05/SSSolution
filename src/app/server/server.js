...
app.post('/login',(req,res)=>{    
    connection.query(`select department_id,role,id,name,role from employee where email = '${req.body.email}'and password = 
    '${req.body.password}'`,(err,record,fields)=>{
        if(err) throw err;
        else{  
            if(record.length>0){      
            res.send(record)
...
app.post('/leaveRequest',(req,res)=>{
    connection.query(`insert into leave_table(userid,subject,reason,reportingPerson_id,dateOfleave,action )
     values(${req.body.leave.userid},'${req.body.leave.subject}','${req.body.leave.reason}','${req.body.leave.reportingPerson_id}',
    '${req.body.leave.dateOfleave}','${req.body.leave.action}')`,(err,result)=>{
        if(err)throw err;
        else{
            res.send({insert:'success'});
        };
